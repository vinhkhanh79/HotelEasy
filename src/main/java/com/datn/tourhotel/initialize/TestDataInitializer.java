    ...
    public void run(String... args) {
        ...
        try {
            ...
            if (roleRepository.count() == 0 && userRepository.count() == 0) {
                ...
                User user5 = User.builder().username("manager3@hotel.com").password(passwordEncoder.encode("1")).name("Nguyễn").lastName("Văn An").role(hotelManagerRole).build();
                ...
                Address addressBerlin3 = Address.builder().addressLine("Budapester Str. 2, Mitte, 10787").city("Berlin")
                        .country("Germany").build();
                Address addressHCM1 = Address.builder().addressLine("50 Lê Lợi, Quận 1").city("Hồ Chí Minh").country("Việt Nam").build();
                Address addressHCM2 = Address.builder().addressLine("35 Nguyễn Huệ, Quận 1").city("Hồ Chí Minh").country("Việt Nam").build();
                Address addressHCM3 = Address.builder().addressLine("22 Lý Tự Trọng, Quận 1").city("Hồ Chí Minh").country("Việt Nam").build();
                ...
                Hotel hotelHCM1 = Hotel.builder().name("Rex Hotel Saigon")
                		.address(addressHCM1).hotelManager(hm3).build();
                Hotel hotelHCM2 = Hotel.builder().name("Park Hyatt Saigon").address(addressHCM2).hotelManager(hm3).build();
                Hotel hotelHCM3 = Hotel.builder().name("Pullman Saigon").address(addressHCM3).hotelManager(hm3).build();
                ...
                Room singleRoomHCM1 = Room.builder().roomType(RoomType.SINGLE)
                		.pricePerNight(3000000).roomCount(20).hotel(hotelHCM1).build();
                Room doubleRoomHCM1 = Room.builder().roomType(RoomType.DOUBLE).pricePerNight(5000000).roomCount(25).hotel(hotelHCM1).build();

                Room singleRoomHCM2 = Room.builder().roomType(RoomType.SINGLE).pricePerNight(3500000).roomCount(15).hotel(hotelHCM2).build();
                Room doubleRoomHCM2 = Room.builder().roomType(RoomType.DOUBLE).pricePerNight(5500000).roomCount(20).hotel(hotelHCM2).build();

                Room singleRoomHCM3 = Room.builder().roomType(RoomType.SINGLE).pricePerNight(2500000).roomCount(30).hotel(hotelHCM3).build();
                Room doubleRoomHCM3 = Room.builder().roomType(RoomType.DOUBLE).pricePerNight(4500000).roomCount(35).hotel(hotelHCM3).build();
